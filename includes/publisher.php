...
class GIW_Publisher
{
    ...
    public function publish()
    {
        ...
        if ($folder != '/' && !empty($folder)) {
            if (strstr($folder, '/') !== false) {
                $repo_structure = $this->get_deep_value($repo_structure, $folder, '/');
            } else {
    ...
    public function get_deep_value($array, $keys, $delimiter = '/')
    {
        $keys = explode($delimiter, $keys);
        $key = array_shift($keys); # 弹出第一个值
        if (count($keys) > 0 && isset($array[$key]['items'])) {
            return self::get_deep_value($array[$key]['items'], implode($delimiter, $keys), $delimiter);
        } else {
            return $array[$key]['items'] ?? null;
        }
    }
}
